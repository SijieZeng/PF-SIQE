...
params.p_stop = 1;    % Probability of stopping in the dilemma zone
params.b_max = 1.5; 
params.d_0 = 0; 
...
% Plot static elements (traffic signals, loop detectors)
subplot(2, 2, 1);
for k = 1:length(S)
    color = color_map(S(k));
    x_start = time_steps_signal(k);
...
for i = 1:num_vehicles
    subplot(2, 2, 1);
    plot(time_steps_vehicle, positions(:, i), 'g-', 'LineWidth', 2, 'DisplayName', sprintf('GT Vehicle %d', i));
    subplot(2, 2, 2);
    plot(time_steps_vehicle, velocities(:, i), 'g-', 'LineWidth', 2, 'DisplayName', sprintf('GT Vehicle %d', i));
    subplot(2, 2, 3);
    plot(time_steps_vehicle, accelerations(:, i), 'g-', 'LineWidth', 2, 'DisplayName', sprintf('GT Vehicle %d', i));
end
...
% 创建一个时间步数组
time_steps = 1:params.num_iterations;
% Run particle filter and update visualization
for t = 1:params.num_iterations
    ...
    % Update weights
    measurement.c_tilde = c_tilde(t);
    measurement.o_tilde = o_tilde(t);
    measurement.v_avg_tilde = v_avg_tilde(t);
    measurement.d_tilde = d_tilde(:, t);
    log_weights = PF.update_weights(particles, measurement, params, M);
    
    % Store particles and weights
    all_particles(:, :, t) = particles;
    all_weights(:, t) = exp(log_weights) / sum(exp(log_weights));

    % Visualize particles
    scaled_weights = all_weights(:, t) * 200000;  % Adjust the multiplier as needed
   % Extract particles for current time step
    current_particles = all_particles(:, :, t);
    
    
    
     % 绘制位置
    subplot(2, 2, 1);
    scatter(repmat(time_steps(t), num_particles, 1), current_particles(:, 1), scaled_weights, particle_colors, 'filled', 'DisplayName', sprintf('Particles t=%d', t));
    hold on;
    
    % 绘制速度
    subplot(2, 2, 2);
    scatter(repmat(time_steps(t), num_particles, 1), current_particles(:, 2), scaled_weights, particle_colors, 'filled', 'DisplayName', sprintf('Particles t=%d', t));
    hold on;
    
    % 绘制加速度
    subplot(2, 2, 3);
    scatter(repmat(time_steps(t), num_particles, 1), current_particles(:, 3), scaled_weights, particle_colors, 'filled', 'DisplayName', sprintf('Particles t=%d', t));
    hold on;
    % Resample
    particles = PF.resample_particles(particles, log_weights, params);
...
legend_elements = [plot(NaN, NaN, 'r.', 'MarkerSize', 20), ...
                   ...
                   plot(NaN, NaN, 'b--', 'LineWidth', 1), ...
                   plot(NaN, NaN, 'g--', 'LineWidth', 1), ...
                   plot(NaN, NaN, '-', 'Color', 'g'), ...
                   scatter([], [], 36, 'filled')];  % 使用scatter来表示粒子
legend(legend_elements, {'Red Signal', 'Yellow Signal', 'Green Signal', ...
                         'Vehicle Count Loop', 'Vehicle Presence Loop', 'Vehicle Speed Loop', ...
...
sgtitle('Ground Truth and Particle Filter Estimation', 'FontSize', 16);
...
if strcmpi(save_choice, 'y')
    ...
    disp(['The particle filter figure has been saved in the folder: ' folder_path]);
else
